...
func getFiles() {
	...
	filepath.Walk(getRoot(), func(path string, info os.FileInfo, err error) error {
		...
		if info.IsDir() == false {
			...
			root := len([]rune(getRoot()))
			dir := string([]rune(path)[root:])
			//BooklistData = append(BooklistData, "[" +strconv.Itoa(num)+"] "+info.Name())
			BooklistData = append(BooklistData, "[" + strconv.Itoa(num) + "] " + dir)
			BookInfos = append(BookInfos, dir)
		}
...
func openBook() {
	...
	b, err := ioutil.ReadFile(getRoot() + BookInfos[curBookIndex])
	if err != nil {
		panic(err)
	}
	str := string(b)
	...
	updateConfig()
	//fmt.Printf("%s=>%s\n",BookInfos[curBookIndex],str)

	initBookScene(BookInfos[curBookIndex], str)
}
